...
class Mainapp(QMainWindow, Ui.Ui_Frame):
    ...
    problems = []
    patient_names_dict = {'Name': [], 'MR': [],'problems':[]}
    Folderpath = ''
    test = 0
    ...
    def check_patiant_MR(self , txt_lines :str ,Patient_No  ,page_num):
        ...
        if index != -1:
            if str(Patient_No) not in str(txt_lines)[index : index+80] :
                Mainapp.problems.append(f'there is problem in Patient MRN at page {page_num}')
    ...
    def check_patiant_name(self , txt_lines :str ,name  ,page_num):
        ...
        if index != -1:
            if str(name) not in str(txt_lines)[index : index+60].replace(',','') :
                Mainapp.problems.append(f'there is problem in Patient Name at page {page_num}')              
    ...
    def write_resualts_in_excel(self):
        ...
        df.to_excel('G:\pdfs\my_excel_sheet.xlsx')
    ...
    def cheack(self , txt ,i,name ,MRN ):
        txt = str(txt)
        #self.check_patiant_name(txt, name ,i)
        self.check_patiant_MR(txt , MRN ,i )
    ...
    def add_to_dict(self,names,MRNs ,problems_Lists):    
        ...
        Mainapp.patient_names_dict['MR'] = MRNs
